...
3. **TUI polling:** `src/hooks/useWorktrees.ts` still polls SQLite + git status (2s default), GitHub PRs (180s default), and Linear tickets (180s default) as a fallback. Uses JSON fingerprinting to skip re-renders when data hasn't changed. Group/flat-list construction is delegated to `src/lib/grouping.ts` (`buildGroups`), shared with the daemon: when `linearGroupByProject` is on (default) and tickets carry a Linear project, worktrees are bucketed project-major (project → repo → ticket), with project-less worktrees in a trailing per-repo section. The invariant is that `flatWorktrees` order always equals the visual row order — headers (project/repo/ticket) are render-only and never consume a selection index.
...
- `src/lib/grouping.ts` — Shared `buildGroups`: sorts worktrees per repo (ticket clustering) and buckets them project-major when `linearGroupByProject` is enabled; used by both `useWorktrees` and the daemon.
